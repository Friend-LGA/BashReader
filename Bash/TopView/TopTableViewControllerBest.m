...
@implementation TopTableViewControllerBest
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.dataArray[indexPath.row] isKindOfClass:[NSString class]])
    {
        ...
        else if ([self.dataArray[indexPath.row] isEqualToString:@"cellInfo"])
        {
            ...
            if (!self.numberOfObjectsInDB) cell.textLabel.text = [NSString stringWithFormat:@"Цитат в базе: %lu", (unsigned long)self.numberOfObjectsInDB];
            else if (indexPath.row == 0) cell.textLabel.text = @"Лучшие цитаты дня";
            else cell.textLabel.text = @"Лучшие цитаты недели";
...
- (CGFloat)tableView:(UITableView *)tableView asyncHeightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.dataArray[indexPath.row] isKindOfClass:[NSString class]])
    {
        ...
        else if ([self.dataArray[indexPath.row] isEqualToString:@"cellInfo"])
        {
            ...
            if (!self.numberOfObjectsInDB) self.prototypeCell2.textLabel.text = [NSString stringWithFormat:@"Цитат в базе: %lu", (unsigned long)self.numberOfObjectsInDB];
            else if (indexPath.row == 0) self.prototypeCell2.textLabel.text = @"Лучшие цитаты дня";
            else self.prototypeCell2.textLabel.text = @"Лучшие цитаты недели";
