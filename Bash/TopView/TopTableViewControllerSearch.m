...
@implementation TopTableViewControllerSearch
    ...
    if ([self.dataArray[indexPath.row] isKindOfClass:[NSString class]])
    {
        ...
        else if ([self.dataArray[indexPath.row] isEqualToString:@"cellInfo"])
        {
            ...
            if (kSettings.loadingFrom == LGLoadingFromInternet)
                cell.textLabel.text = [NSString stringWithFormat:@"Найдено цитат: %lu", (unsigned long)self.dataArray.count-1];
            else
...
- (CGFloat)tableView:(UITableView *)tableView asyncHeightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.dataArray[indexPath.row] isKindOfClass:[NSString class]])
    {
        ...
        else if ([self.dataArray[indexPath.row] isEqualToString:@"cellInfo"])
        {
            ...
            if (kSettings.loadingFrom == LGLoadingFromInternet)
                self.prototypeCell2.textLabel.text = [NSString stringWithFormat:@"Найдено цитат: %lu", (unsigned long)self.dataArray.count-1];
            else
