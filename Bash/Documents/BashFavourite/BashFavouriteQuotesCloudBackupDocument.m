...
static NSString *const  kVersionKey     = @"bashFavouriteQuotesCloudBackupDocumentVersion";
static NSString *const  kArrayKey       = @"bashFavouriteQuotesCloudBackupDocumentArray";
static int       const  kCurrentVersion = 1;

@implementation BashFavouriteQuotesCloudBackupDocument
...
- (BOOL)loadFromContents:(id)contents ofType:(NSString *)typeName error:(NSError **)outError
{
    if (contents)
    {
        if ([contents length] > 0)
        {
            ...
            {
                NSLog(@"BashFavouriteQuotesCloudBackupDocument: Error while trying to unarchive array, exception - %@", exception);
